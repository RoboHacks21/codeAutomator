...
def getFreqForNote(note):
    freq = notesSheet[notesSheet["LetterRep"] == note].iloc[0][1]
    return freq

def playNote(note):
    freq = getFreqForNote(note)
    print(freq)
    sine_wave = get_sine_wave(freq, duration=2, amplitude=2048)
    wavfile.write('note.wav', rate=44100, data=sine_wave.astype(np.int16))

    AudioPlayer.start_music("note.wav", 0.20)

# Pure sine wave
def playNotesFromFreqArr(notesArray):
    # print("playing song")
    # Pure sine wave
...
def playFromArray(notesArray, timingArray):
    print("playing song")
    for i in range(len(notesArray)):
        sine_wave = get_sine_wave(getFreqForNote(notesArray[i]), duration=2, amplitude=2048)
        wavfile.write('note.wav', rate=44100, data=sine_wave.astype(np.int16))

        AudioPlayer.start_music("note.wav", timingArray[i] / 4)
